...
flowchart LR
  ...
  RAW --> BASE["轻量评分修订<br/>保留 available_at 时点语义"]
...
- 实际数值、单位、质量状态；
- 观测时间、可用时间和来源；
- 按 `available_at + source` 保留轻量修订，保证历史 `asOf` 重放不会读取未来更正。

不保存 `payload_json`，只保存评分所需字段。日常最新日评分在数据初始化完成并启用开关后读取该表，避免扫描大 JSON；同一交易日的修订仍按时点过滤。
...
归档能力默认关闭，先创建表、回填和校验基线，再由运维显式启用。五年回填与审计启用分层读取后查询热表和冷表；日常评分读取无原始 JSON 的轻量修订。
...
每条新观测继续幂等写入热原始表，并同步 upsert 到轻量修订表。双写在同一事务内完成：热表保留全部修订与 JSON，轻量表保留全部时点修订但不保存 JSON。
...
- 新观测对热表和基线表双写幂等，不同 `available_at` 修订分别保留，历史 `asOf` 可重复计算。
