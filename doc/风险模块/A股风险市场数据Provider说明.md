...
| `market_daily` | 开收盘、成交量、基准和龙头收盘序列 | `V3`、`V4`、`C1`、`C3`、`C4`、`C5` | 超额收益、成交拥挤、龙头相对收益、下跌量比、相对强弱、趋势和缺口代理 |
| `breadth` | 上涨/下跌、新高/新低、均线上方家数 | `C2` | 输出三个市场宽度分量，不在 Provider 内合成最终分 |
| `cross_market` | 领先资产收益、动态相关、独立市场确认数 | `S1`、`S2`、`S4` | 输出标准化收益、相关系数和跨市场确认比例 |

指标权重沿用风险框架评分卡，仅用于 `MarketRiskCoverageReport` 的本数据集加权覆盖报告。报告只描述本次请求的实际 `availability/quality`；是否达到正式评分的 80% 总覆盖及 V、C、A、T/S 必备证据门槛，由风险引擎统一决定。
