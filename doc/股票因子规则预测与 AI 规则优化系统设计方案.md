...
| 序号 | 任务模块 | 状态 |
|---|---|---|
| 1 | 项目基础架构 | 已完成（M0 基础契约，提交 `a56a8e6`） |
| 2 | 行情与新闻数据采集 | 已完成一期 A 股免 Token 行情接入；新闻采集待后续实现 |
| 3 | 技术因子计算 | 待合并（当前 worktree 未发现服务层实现） |
| 4 | Drools 规则推理 | 待合并（当前 worktree 未发现服务层实现） |
| 5 | 信号输出与规则触发记录 | 待合并（当前 worktree 未发现服务层实现） |
| 6 | 预测结果验证 | 待合并（当前 worktree 未发现服务层实现） |
| 7 | AI 复盘分析 | 待合并（当前 worktree 未发现服务层实现） |
| 8 | 候选规则生成 | 待合并（当前 worktree 未发现服务层实现） |
| 9 | 候选规则回测 | 待合并（当前 worktree 未发现服务层实现） |
| 10 | 人工审核与规则上线 | 未开始 |
| 11 | 调度与集成验收 | 已完成调度骨架与手动触发契约；完整端到端链路需等待 M1-M6/M9 合并 |

## 18.1 M7 集成依赖清单

当前 worktree 中多个阶段分支均指向 `a56a8e6`，尚未合入 M1-M6 的服务层业务能力。M7 不凭空补齐缺失业务，只提供安全的每日任务编排骨架、手动触发接口和默认禁用的 Spring Scheduler 配置。

| 依赖模块 | 需要合入的能力 | 冲突风险 |
|---|---|---|
| M1 行情与新闻数据采集 | 行情导入、新闻/宏观采集、交易日历、幂等导入 | 外部数据源配置、限流、失败重试和调度时间窗口需要统一 |
| M2 技术因子计算 | 按交易日和股票代码计算并落库因子快照 | 因子 JSON 结构、空行情处理、未来数据防泄露口径需要统一 |
| M3 Drools 规则推理 | 读取启用规则和因子快照，输出规则命中结果 | 规则状态、优先级、冲突解决和异常隔离策略需要统一 |
| M4 信号输出与触发记录 | 生成看涨、看跌、观望、高风险等辅助决策信号 | 所有信号必须保留风险提示，不得表达确定性预测或收益保证 |
| M5 预测结果验证与回测 | 验证历史信号表现，支持规则/候选规则回测 | 回测对象类型、时间切片、样本外验证和指标口径需要统一 |
| M6 AI 复盘与候选规则 | 结构化复盘、候选规则生成、可追溯建议 | AI 输出不能直接上线规则，候选规则必须进入回测和人工审核 |

后续建议合并顺序：

1. 先合并 M1，确认行情和基础数据可稳定落库。
2. 合并 M2，基于已落库行情补齐因子快照。
3. 合并 M3 和 M4，形成从因子到辅助决策信号的最小闭环。
4. 合并 M5，补齐历史验证和候选规则回测。
5. 合并 M6，接入 AI 复盘和候选规则生成。
6. 最后回到 M7，将各模块服务注册为调度步骤处理器，补充真实端到端冒烟测试。

## 18.2 A 股免 Token 行情运行说明

一期使用 AKTools `0.0.91` 与 AKShare `1.18.64`，页面只读取本地 Spring Boot API 和 MySQL，不在用户查询期间直接访问公开行情网站。公开上游异常时同步任务记录失败并保留最后一次成功数据，禁止静默回退到 `mock`。
...
默认使用国内可达的 PyPI 镜像构建；可通过环境变量 `PIP_INDEX_URL` 替换。AKTools 只绑定 `127.0.0.1:8090`，Spring Boot 默认通过 `AKTOOLS_BASE_URL=http://127.0.0.1:8090` 访问。

仅当 Docker 19 等旧运行时因 seccomp 无法创建 Python 工作线程时，使用兼容覆盖文件；现代 Docker 保持默认隔离：

```bash
docker-compose -f docker-compose.market-data.yml \
  -f docker-compose.market-data.legacy.yml up -d --build
```

首次初始化：

```bash
curl --fail -X POST 'http://127.0.0.1:8080/api/market-data/sync/bootstrap?market=A%E8%82%A1'
```

初始化顺序为股票列表、交易日历、最近交易日全市场收盘快照、沪深 300 最近 120 个自然日行情。初始化完成后，添加股票抽屉通过后端分页搜索全部沪深京股票；新加入股票即使尚未执行规则，也会在看板显示“待生成信号”。

每日收盘任务默认使用 `0 0 18 * * MON-FRI`，生产环境设置 `DAILY_WORKFLOW_ENABLED=true` 后启用。信号始终用于股票研究和辅助决策，不构成投资建议，不代表确定性预测，也不保证收益。

当前稳定的新浪股票列表与快照接口不提供行业分类。同步时保留数据库已有行业，不用名称猜测或伪造分类；行业为空的股票不参与行业强弱统计。后续可通过独立行业分类 Provider 补齐，不影响股票列表、收盘行情、沪深 300与信号看板主链路。

2026-07-12 本地真实数据验收结果：基础股票 `5530` 只、2026-07-10 有效收盘快照 `5526` 条、沪深 300 日线 `82` 条；`600519.SH` 与 `300750.SZ` 均可搜索。公开快照中 4 条源数据违反“收盘价不高于最高价”的质量规则，被拒绝且记录在同步运行错误中，未写入伪造或修正后的行情。
